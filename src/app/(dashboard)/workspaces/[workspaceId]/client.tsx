...
import { PageLoaader } from "@/components/page-loader";
...
export const WorkspaceIdClient = () => {
  ...
  if (isLoading) return <PageLoaader />;
  ...
  return (
    <div className="flex flex-col gap-y-4 col-span-1">
      <div className="bg-muted rounded-lg p-4">
        ...
        <DottedSeparator className="my-4" />
        <ul className="gird grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {data.map((member) => (
            <li key={member.$id}>
              <Card className="shadow-none rounded-lg overflow-hidden">
                <CardContent className="p-3 flex flex-col items-center gap-x-2">
                  <MemberAvatar className="size-12" fallbackClassName="text-lg" name={member.name} />
                  <div className="flex flex-col items-center overflow-hidden">
                    <p className="text-lg font-medium line-clamp-1">{member.name}</p>
                    <p className="text-lg text-muted-foreground line-clamp-1">{member.name}</p>
                  </div>
                </CardContent>
              </Card>
            </li>
